      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {draft.checkIn
              ? `${formatRange(draft.checkIn, draft.checkOut)}${nights ? ` · ${nights} nights` : ""}`
              : `from ${formatEuro(room?.nightly ?? property.fromNightly)} / night`}
          </p>
        </div>
        <Link
          to={to}
          className="eyebrow shrink-0 bg-primary px-5 py-3 text-primary-foreground"
        >
          {quote ? formatEuro(quote.total) : cta}
        </Link>
      </div>
    </div>
  );
...
export function JourneySteps({ current }: { current: 1 | 2 | 3 | 4 }) {
  const steps = ["House", "Dates", "Details", "Payment"];
  return (
  ...
  return (
    <dl className="mt-6 space-y-3 border-t border-border pt-6 text-sm">
      <div className="flex justify-between gap-4 text-muted-foreground">
        <dt>
          {formatEuro(nightly)} × {nights} {nights === 1 ? "night" : "nights"}
        </dt>
      ...
      <div className="flex justify-between border-t border-border pt-3 font-serif text-2xl">
        <dt>Total</dt>
