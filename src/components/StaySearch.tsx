...
import { isoDate, addDays, parseIso, startOfDay } from "@/lib/utils";
...
export function StaySearch({ variant = "hero" }: Props) {
  ...
  const minOut = checkIn ? isoDate(addDays(parseIso(checkIn), 1)) : isoDate(addDays(today, 1));
  ...
  const onSubmit = (event: FormEvent) => {
    ...
    navigate(`${paths.residences}?${params.toString()}`);
  };
