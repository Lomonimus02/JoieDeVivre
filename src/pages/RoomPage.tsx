...
export function RoomPage() {
  ...
  if (!property || !room) return <Navigate to="/stays" replace />;
  ...
  return (
    <div className="pb-28 lg:pb-16">
      ...
      <div className="mx-auto max-w-[86rem] px-6 pt-10 lg:px-10">
        <Link to={`/stays/${property.slug}`} className="eyebrow text-muted-foreground">
          ← {property.name}
        </Link>
      ...
      <StickyBookBar to={`/stays/${property.slug}/dates?room=${room.slug}`} cta="Select dates" />
    </div>
  );
}
